...
void AGeometryHubActor::Tick(float DeltaTime)
{
	...
	UE_LOG(LogGeometryHub, Warning, TEXT("property pointer %i, is valid %i"),
		PropertyActor != nullptr, IsValid(PropertyActor));
	UE_LOG(LogGeometryHub, Error, TEXT("none property pointer %i, is valid %i"),
		NonePropertyActor != nullptr, IsValid(NonePropertyActor));
}
...
void AGeometryHubActor::DoActorSpawn1()
{
	...
	if (World) {
		for (int32 i = 0; i < 10; i++) {
			const FTransform GeometryTransform = FTransform(FRotator::ZeroRotator, FVector(0.0f, 150.0f * i, 300.0f));
...
void AGeometryHubActor::DoActorSpawn2()
{
	...
	if (World) {
		for (int32 i = 0; i < 10; i++) {
			const FTransform GeometryTransform = FTransform(FRotator::ZeroRotator, FVector(0.0f, 150.0f * i, 600.0f));
...
void AGeometryHubActor::DoActorSpawn4()
{
	UWorld* World = GetWorld();
	if (World) {
		FTransform GeometryTransform = FTransform(FRotator::ZeroRotator, FVector(0.0f, 150.0f, 300.0f));
		NonePropertyActor = World->SpawnActor<ABaseGeometryActor>(GeometryClass, GeometryTransform);

		GeometryTransform = FTransform(FRotator::ZeroRotator, FVector(0.0f, 300.0f, 300.0f));
